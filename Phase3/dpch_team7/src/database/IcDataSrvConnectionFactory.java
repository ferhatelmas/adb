...
public class IcDataSrvConnectionFactory {
  ...
  int nationAndRegionServerIndex = 6;
  ...
  private Connection makeTestConnection() throws InstantiationException, IllegalAccessException,
      ClassNotFoundException, SQLException {
    String url = baseConnectionUrl + localhost + ":" + port + "/";
    Class.forName(driver).newInstance();
    Connection conn = (Connection) DriverManager.getConnection(url + getDbName()
        + "?group_concat_max_len=100000", testUserName, testPassword);
  ...
  public List<Connection> getAllConnections() {
    ...
    // The server is dead! connections.add(icdatasrv1_18);
  ...
  private Connection makeConnection(int node, int zone) throws InstantiationException,
      IllegalAccessException, ClassNotFoundException, SQLException {
    Connection conn;
    String url = baseConnectionUrl + host + node + "-" + zone + ":" + port + "/";
    Class.forName(driver).newInstance();
    conn = (Connection) DriverManager.getConnection(url + dbName, userName, password);
    // System.out.println("Connection to " + host + node + "-" + zone +
    // " successful");
    return conn;
  }
