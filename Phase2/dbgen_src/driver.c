...
usage (void)
{
	fprintf (stderr, "%s\n%s\n\t%s\n%s %s\n\n",
		"USAGE:",
		"dbgen [-{vf}][-T {pcsoPSOL}]",
		"[-s <scale>][-C <procs>][-S <step>] [-p <partition>]",
		"dbgen [-v] [-O m] [-s <scale>] [-p <partition>]",
		"[-U <updates>]");
	...
	fprintf (stderr, "-S <n> -- build the <n>th step of the data/update set (used with -C or -U)\n");
        fprintf (stderr, "-p <n> -- generate data for the <n>th partition (1..8; fixed at eight partitions)\n");
