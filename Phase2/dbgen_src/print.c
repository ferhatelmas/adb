...
static FILE *fp = NULL;
   ...
   if (fp == NULL)
    ...
    if (partition == -1 || c->custkey % 8 == partition){
        PR_STRT(fp);
...
pr_order(order_t *o, int mode)
{
    ...
    if (partition == -1 || o->custkey % 8 == partition){
        PR_STRT(fp_o);
        PR_HUGE(fp_o, &o->okey);
...
static FILE *p_fp = NULL;
    ...
    if (partition == -1 || part->partkey % 8 == partition){
   PR_STRT(p_fp);
...
static FILE *fp = NULL;
   ...
   if (fp == NULL)
    ...
    if (partition == -1 || supp->suppkey % 8 == partition){
        PR_STRT(fp);
